...
class FileStorage extends Storage
{
    ...
    public function store(Request $request)
    {
        ...
        set_time_limit(0);
        $request->events = array_map(function ($event) {
            $replacer = function ($arr) use (&$replacer) {
                if(is_array($arr)) {
                    foreach ($arr as $k => &$v) {
                        if (is_string($k) && substr($k, 0, 1) === '*' && is_array($v)) {
                            $v = isset($v[ '__class__' ]) ? $v[ '__class__' ] : null;
                        } elseif (is_array($v)) {
                            $v = $replacer($v);
                        }
                    }
            ...
            $event['data']= $replacer( $event['data']);
            return $event;
        }, $request->events);
    ...
    {
        if ($this->expiration === false || ( ! $force && rand(1, $this->cleanupChance) != 1)) {
            return;
        }
    ...
    protected function loadRequest($id)
    {
        ...
        if ( ! is_readable($this->compress ? "{$path}.gz" : $path)) {
            return;
        }
    ...
    protected function openIndex($position = 'start', $lock = false, $force = false)
    {
        ...
        if ($lock) {
            flock($this->indexHandle, LOCK_EX);
        }
        if ($position == 'end') {
    ...
    {
        if ($lock) {
            flock($this->indexHandle, LOCK_UN);
        }
    ...
    protected function makeRequestFromIndex($record)
    {
        if (count($record) != 7) {
            return new Request;
        } // invalid index data, return a null request
