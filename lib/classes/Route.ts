...
export class Route extends String {
    ...
    constructor(name, params, absolute) {
        ...
        this.urlBuilder = this.name ? new UrlBuilder(name, absolute) : null;
    ...
    current(name = null) {
        ...
        let currentRoute = routeNames.filter(name => {
            if (app.routes[name].methods.indexOf('GET') === -1) {
                return false;
            }
    ...
    get params() {
        const namedRoute = app.routes[this.current() as any];
...
export function route(name, params?:any, absolute:boolean=false) {
    return new Route(name, params, absolute);
}
